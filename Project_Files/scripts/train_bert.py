"""
BERT Model Training Module for Job Advertisement Zone Classification

This module trains a BERT-based token classification model to identify different zones
within job advertisements (e.g., skills, requirements, benefits). Implements training
with class-weighted loss, early stopping, and TensorBoard logging.
...
Output:
    - best_model.pt: Best model state dict based on validation loss
    - Model config and weights via save_pretrained()
...
def train():
    ...
    # Determine training configuration based on GPU availability
    training_epochs = EPOCHS  # Default to full epochs
    
    if not torch.cuda.is_available():
        print("WARNING: GPU not found. Switching to CPU mode.", flush=True)
        print("Optimization: Using a subset of data (5%) and fewer epochs to ensure pipeline completion.", flush=True)
        # Subset data
        ...
        training_epochs = 3  # Reduce epochs for CPU
    ...
    for epoch in range(training_epochs):
        print(f"\n{'='*60}", flush=True)
        print(f"Epoch {epoch + 1}/{training_epochs}", flush=True)
        ...
        print(f"Average train loss: {avg_train_loss:.4f}", flush=True)
        ...
        print(f"Validation Accuracy: {avg_val_acc:.4f}", flush=True)
        ...
        if avg_val_loss < best_val_loss:
            ...
            patience_counter = 0
            # Save best model
            print("✓ New best model! Saving...", flush=True)
            ...
            patience_counter += 1
            print(f"Early stopping counter: {patience_counter}/{EARLY_STOPPING_PATIENCE}", flush=True)
    ...
    print(f"✓ Best model saved to: {MODEL_DIR}", flush=True)
    print("="*60, flush=True)
    writer.close()
